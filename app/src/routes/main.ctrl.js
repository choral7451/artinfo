...
const output = {
  ...
  recruit_art_orchestra: async (req, res) => {
    ...
    if (auth != undefined) {
      logger.info(
        "GET /recruit_art/orchestra 304 모집공고(예술단체) 화면으로 이동"
      );
  ...
  recruit_art_choir: async (req, res) => {
    ...
    if (auth != undefined) {
      logger.info(
        "GET /recruit_art/choir 304 모집공고(예술단체) 화면으로 이동"
      );
  ...
  recruit_art_administration: async (req, res) => {
    ...
    if (auth != undefined) {
      logger.info(
        "GET /recruit_art/administration 304 모집공고(예술단체) 화면으로 이동"
      );
      res.render("recruit_art_administration", {
        login: auth.user.id,
        arrayData,
      });
    } else {
  ...
  recruit_religion_content: async (req, res) => {
    ...
    if (auth != undefined) {
      logger.info(
        "GET /recruit_religion/content 304 모집공고(종교) 화면으로 이동"
      );
      res.render("recruit_religion_content", {
        login: auth.user.id,
        data,
        content,
      });
  ...
  recruit_religion_conductor: async (req, res) => {
    ...
    if (auth != undefined) {
      logger.info(
        "GET /recruit_religion/conductor 304 모집공고(종교) 화면으로 이동"
      );
      res.render("recruit_religion_conductor", {
        login: auth.user.id,
        arrayData,
      });
    } else {
  ...
  recruit_religion_solists: async (req, res) => {
    ...
    if (auth != undefined) {
      logger.info(
        "GET /recruit_religion/solists 304 모집공고(종교) 화면으로 이동"
      );
      res.render("recruit_religion_solists", {
        login: auth.user.id,
        arrayData,
      });
    } else {
  ...
  recruit_religion_accompanist: async (req, res) => {
    ...
    if (auth != undefined) {
      logger.info(
        "GET /recruit_religion/accompanist 304 모집공고(종교) 화면으로 이동"
      );
      res.render("recruit_religion_accompanist", {
        login: auth.user.id,
        arrayData,
      });
    } else {
  ...
  recruit_religion_write: async (req, res) => {
    ...
    if (auth) {
      logger.info(
        "GET /recruit_religion/write 304 모집공고(종교)/글쓰기 화면으로 이동"
      );
  ...
  recruit_religion_update: async (req, res) => {
    ...
    if (auth != undefined) {
      if (auth.user.id == data[0].WRITER) {
        logger.info(
          "GET /recruit_religion/update 304 모집공고(종교) 화면으로 이동"
        );
        res.render("recruit_religion_update", {
          login: auth.user.id,
          data,
          content,
        });
  ...
  ads: (req, res) => {
    res.type("text/plain");
    res.send("google.com, pub-8515708471646582, DIRECT, f08c47fec0942fa0");
  },
